...
class HashMap {
 private:
    ...
    /* This method rebuilds table when quantity of elements becomes more 
     than 2/3 of hash table capacity. Time: O(quantity of elements in the table). */
    void reallocate() {
        if (3 * value_store_.size() >= hashed_pointers_.size() * 2) {
            size_t new_size = hashed_pointers_.size() * 2;
    ...
    /*Returns constant iterator to the element if this element is in the table ot constant iterator end() otherwise.
    Time: randomized O(1).*/
 ...
 public:
    ...
    class iterator {
     ...
     public:
        ...
        bool operator ==(const iterator& other) const {
            return ((index_ == other.index_) && (my_hashmap_ == other.my_hashmap_));
        }
    ...
    //This is constant forward iterator class. It helps to iterate in the data structure. 
    //Also it is good alternative to constant pointers.
    ...
    //Returns true if and only if there is no elements in hash table. Time: O(1).
    ...
    //Cconstant iterator points behind the last element in hash_table. Time: O(1).
    const_iterator end() const {
        return {value_store_.size(), this};
    }

    //Returns iterator to the element if this element is in the table ot iterator end() otherwise. Time: randomized O(1). 
    ...
    //Removes element from the hash_table. The size of storage doesn't change. Time: randomized O(1).*/
    void erase(const KeyType& key) {
        if (find(key) == end()) {
            ...
            size_t hash1 =
                    hasher_(value_store_.back().first) % hashed_pointers_.size();
    ...
    /*Inserts element into the hash table only if there was not such element. Calls reallocate if necessary.
    Time: randomized and amortized O(1). O(quantity of elements in the table) while reallocation.*/
