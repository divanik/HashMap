...
public:
    ...
    public:
        ...
        std::pair<const KeyType, ValueType>& operator*() const {
            return reinterpret_cast<std::pair<const KeyType, ValueType>&>(my_hashmap->value_store[index]);
        }
        std::pair<const KeyType, ValueType>* operator->() const {
            return reinterpret_cast<std::pair<const KeyType, ValueType>*>(&my_hashmap->value_store[index]);
        }
    ...
    public:
        ...
        const_iterator(size_t index_ = 0, const HashMap* my_hashmap_ = nullptr) :
            index(index_), my_hashmap(my_hashmap_) {}
        ...
        const std::pair<const KeyType, ValueType>& operator*() const {
            return reinterpret_cast<const std::pair<const KeyType, ValueType>&>(my_hashmap->value_store[index]);
        }
        const std::pair<const KeyType, ValueType>* operator->() const {
            return reinterpret_cast<const std::pair<const KeyType, ValueType>*>(&my_hashmap->value_store[index]);
        }
    ...
    HashMap(const Hash& hasher_ = Hash()) : value_store(0), hashed_pointers(1, vector<size_t>(0)), hasher(hasher_){}

    template<class Forward_Iter>
    HashMap(Forward_Iter first, Forward_Iter last, const Hash& hasher_ = Hash()) :
        value_store(0), hashed_pointers(1, std::vector<size_t>(0)), hasher(hasher_) {
        for (; first != last; first++) {
            insert(*first);
        }
    }

    HashMap(std::initializer_list<std::pair<KeyType, ValueType>> init_list, const Hash& hasher_ = Hash())
        : value_store(0), hashed_pointers(1, vector<size_t>(0)), hasher(hasher_)  {
    ...
    iterator find(const KeyType& key) {
        ...
        //cout << "find" << " " << current_hash << " " << hashed_pointers.size() << " " << hashed_pointers[0].size() << endl;
        return { value_store.size(), this };
    }
    ...
    void erase(const KeyType& key) {
        if (find(key) == end()) {
            ...
            size_t hash1 = hasher(value_store.back().first) % hashed_pointers.size();
